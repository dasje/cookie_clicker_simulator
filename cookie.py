...
class ClickerState:
    ...
    def time_until(self, cookies):
        ...
        """
        # print 'time until func'
        if cookies <= self.get_cookies():
            #    print 'target cookies less than current cookies'
            return 0.0
        # print 'cookie trarget', cookies
        # print 'current cookies', self.get_cookies()
        # print 'current cps', self.get_cps()
        time_to_wait = math.ceil((cookies - self.get_cookies()) / self.get_cps())
        # print 'time to wait to', time_to_wait
        return time_to_wait
    ...
    def buy_item(self, item_name, cost, additional_cps):
        ...
        """
        # print 'inside buy item func'
        if cost > self.get_cookies():
            # print 'cost greater than cookies'
            return
        else:
...
def simulate_clicker(build_info, duration, strategy):
    ...
    while True:
        ...
        if cso.get_time() > duration:
            #    print 'break loop, time greater than duration'
            break
        # call strategy function to determine next item to purchase
        time_left = duration - cso.get_time()
        # print 'time left', time_left
        # print '\nfirst - get next purchase'
        # print cso.get_cookies(), type(cso.get_cookies())
        # print cso.get_cps(), type(cso.get_cps())
        # print cso.get_history(), type(cso.get_history())
        # print time_left, type(time_left)
        # print build_info_clone, type(build_info_clone)
        next_purchase = strategy(cso.get_cookies(), cso.get_cps(), cso.get_history(), time_left, build_info_clone)
        # print 'next purchase', next_purchase
        # break out of loop if strategy returns None
        if next_purchase == None:
            #    print '\nsecond - break loop'
            break
        # get wait time until next possible purchase
        # print '\nthird - get the waiting time'
        # print 'build info', build_info_clone.get_cost(next_purchase)
        ...
        # print '\nfourth - wait'
        # print 'time, waittime', cso.get_time(), wait_time
        wait_to = cso.get_time() + wait_time
        # print 'wait until', wait_to
        if wait_to > duration:
            #    print '\nfifth - break loop'
            break
        ...
        # print '\nsixth - wait'
        if wait_time == 0:
            #    print 'wait time is 0'
            pass
        else:
            # check if wait fun argument is time to wait to or time to wait
            #    print 'time before wait', cso.get_time()
            cso.wait(wait_time)
        #    print 'time after wait', cso.get_time()
        # buy the item
        # print '\nseventh - buy item'
        ...
        # print '\neigth - update build clone'
        build_info_clone.update_item(next_purchase)
        # print 'new item cost', build_info_clone.get_cost(next_purchase)
    # outside of loop
    # allow any time left to go past
    # print '\nninth'
    # print 'duration, time', duration, cso.get_time()
    cso.wait(duration - cso.get_time())
    # print 'current time at end of loop', cso.get_time()
    ...
    while True:
        ...
        if next_purchase == None:
            ...
            #        print '\ntwelth'
            break
    # purchases can be made on the final second too
    # multiple purchases can be made at the same time
    return cso
...
def strategy_cheap(cookies, cps, history, time_left, build_info):
    ...
    # return cheapest item
        ...
        print
        sorted(available_items, key=lambda d_x: d_x[0])[0][1]
...
def strategy_expensive(cookies, cps, history, time_left, build_info):
    ...
    print
    'potential cookies', potential_cookies
    # get available items
    # get costs for available items
    available_items = []
    for item in build_info.build_items():
    ...
    # return `most expensive item
        ...
        print
        sorted(available_items, key=lambda d_x: d_x[0], reverse=True)[0][1]
...
# def ratio(mip, cpsr):
#    """
#    Returns the ratio between number of purchases available and
#    the increase in cps.
#    """
#    return cpsr / mip
